...
if (!empty($_POST["nome"]) && !empty($_POST["descricao"]) && !empty($_POST["website"]) && !empty($_POST["diretor"]) && !empty($_POST["email"]) && !empty($_POST["telefone"]) && !empty($_POST["regiao"]) && !empty($_POST["duracao"]) && !empty($_POST["ects"]) && !empty($_POST["regime"]) && !empty($_POST["idioma"]) && !empty($_POST["propina"]) && !empty($_POST["certificacao"]) && !empty($_POST["destinatarios"]) && !empty($_POST["vagas"]) && !empty($_POST["periodo"]) && !empty($_POST["requisitos"]) && !empty($_POST["curricular"]) && !empty($_POST["vocacional"]) && !empty($_POST["atividades"]) && !empty($_POST["apoios"]) && !empty($_POST["alojamento"])) {
    ...
    $query = "INSERT INTO courses (name_course, description_course, website_course, facebook_course, instagram_course, course_director, email_course, phone_course, duration_course, credits_ects, languages, course_fee, certification, target, number_vac, stages, requirements, curriculum_plan, vocational_dimension, support, activities, course_regime_idcourse_regime, accommodation_idaccommodation, users_idusers, region_idregion) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    if (mysqli_stmt_prepare($stmt, $query)) {
        ...
        VALUES (?, ?)";
            if (mysqli_stmt_prepare($stmt, $query3)) {
                ...
                foreach ($_POST["area"] as $idareas) {
                    if (!mysqli_stmt_execute($stmt)) {
                        header("Location:../pt/pages/uploadCourseHei.php");
                        $_SESSION["course"] = 2;
                    }
                }
            }
        ...
        header("Location:../pt/pages/allCoursesHeis.php");
        $_SESSION["course"] = 1;
    } else {
        header("Location:../pt/pages/uploadCourseHei.php");
        $_SESSION["course"] = 2;
