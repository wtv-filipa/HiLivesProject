...
if (isset($_SESSION["idUser"]) && !empty($_POST["nomeuc"]) && !empty($_POST["uniuc"]) && !empty($_POST["data"])) {
    ...
    $query = "INSERT INTO done_cu (cu_name_en, university_name_en, date_cu, users_idusers) VALUES (?,?,?,?)";
    ...
    if (mysqli_stmt_prepare($stmt, $query)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            header("Location: ../en/pages/profile.php?user=$users_idusers");
            $_SESSION["profile"] = 3;
        } else {

            header("Location: ../en/pages/uploadCourse.php");
            $_SESSION["doneCU"] = 1;
        }
    } else {
