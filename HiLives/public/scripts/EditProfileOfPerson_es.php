...
$query = "UPDATE users
SET work_xp_es = ?, description_es = ?, educ_lvl_ideduc_lvl = ?, edit_request = ?, status_edit = ?
WHERE idusers = ?";
...
if (isset($_GET["person"]) && !empty($_POST["esc"]) && !empty($_POST["area"]) && !empty($_POST["exp_t"]) && !empty($_POST["capacity"]) && !empty($_POST["environment"]) && !empty($_POST["def"])) {
    ...
    if (mysqli_stmt_prepare($stmt, $query)) {
        ...
        if (!mysqli_stmt_execute($stmt)) {
            header("Location: ../es/pages/editProfileTutor.php?edit=$idUser");
            ...
            //SUCESS
            header("Location: ../es/pages/IndividualReqEditTutor.php?edit=$idUser");
            $_SESSION["edit"] = 1;
