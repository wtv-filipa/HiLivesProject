...
SET name_course_is = ?, description_course_is = ?, website_course = ?, facebook_course = ?, instagram_course = ?, course_director = ?, email_course = ?, phone_course = ?, duration_course_is = ?, credits_ects_is = ?, languages_is = ?, course_fee_is = ?, certification_is = ?, target_is = ?, number_vac = ?, stages_is = ?, requirements_is = ?, curriculum_plan_is = ?, vocational_dimension_is = ?, support_is = ?, activities_is = ?, course_regime_idcourse_regime = ?, accommodation_idaccommodation = ?
WHERE idcourses = ?";
...
if (isset($_GET["course"]) && !empty($_POST["nome"]) && !empty($_POST["descricao"]) && !empty($_POST["website"]) && !empty($_POST["diretor"]) && !empty($_POST["email"]) && !empty($_POST["telefone"]) && !empty($_POST["duracao"]) && !empty($_POST["ects"]) && !empty($_POST["regime"]) && !empty($_POST["idioma"]) && !empty($_POST["propina"]) && !empty($_POST["certificacao"]) && !empty($_POST["destinatarios"]) && !empty($_POST["vagas"]) && !empty($_POST["periodo"]) && !empty($_POST["requisitos"]) && !empty($_POST["curricular"]) && !empty($_POST["vocacional"]) && !empty($_POST["atividades"]) && !empty($_POST["apoios"]) && !empty($_POST["alojamento"])) {
    ...
    if (mysqli_stmt_prepare($stmt, $query)) {
        ...
        if (!mysqli_stmt_execute($stmt)) {
            header("Location: ../is/pages/editCourseHei.php?course=$idCourse");
            $_SESSION["course"] = 1;
            echo "erro primeiro";
        } else {
