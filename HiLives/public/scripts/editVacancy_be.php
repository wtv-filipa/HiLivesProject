...
SET vacancy_name_be = ?, description_vac_be = ?, free_vac = ?, requirements_be = ?, region_idregion = ?, workday_idworkday = ?, educ_lvl_ideduc_lvl = ?, areas_idareas = ?
WHERE idvacancies = ?";
...
if (!empty($_GET["vac"]) && !empty($_POST["nomevaga"]) && !empty($_POST["descricao"]) && !empty($_POST["numvagas"]) && !empty($_POST["requisitos"]) && !empty($_POST["area"]) && !empty($_POST["jornada"]) && !empty($_POST["educ"]) && !empty($_POST["regiao"])) {
    ...
    if (mysqli_stmt_prepare($stmt, $query)) {
        ...
        if (!mysqli_stmt_execute($stmt)) {
            header("Location: ../be/pages/editVacancy.php?vac=$vacancy_id");
            $_SESSION["vac"] = 1;
        } else {
