...
if (!empty($_POST["nome"]) && !empty($_POST["descricao"]) && !empty($_POST["website"]) && !empty($_POST["diretor"]) && !empty($_POST["email"]) && !empty($_POST["telefone"]) && !empty($_POST["regiao"]) && !empty($_POST["duracao"]) && !empty($_POST["ects"]) && !empty($_POST["regime"]) && !empty($_POST["idioma"]) && !empty($_POST["propina"]) && !empty($_POST["certificacao"]) && !empty($_POST["destinatarios"]) && !empty($_POST["vagas"]) && !empty($_POST["periodo"]) && !empty($_POST["requisitos"]) && !empty($_POST["curricular"]) && !empty($_POST["vocacional"]) && !empty($_POST["atividades"]) && !empty($_POST["apoios"]) && !empty($_POST["alojamento"])) {
    ...
    $query = "INSERT INTO courses (name_course_be, description_course_be, website_course, facebook_course, instagram_course, course_director, email_course, phone_course, duration_course_be, credits_ects_be, languages_be, course_fee_be, certification_be, target_be, number_vac, stages_be, requirements_be, curriculum_plan_be, vocational_dimension_be, support_be, activities_be, course_regime_idcourse_regime, accommodation_idaccommodation, users_idusers, region_idregion) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        ...
        VALUES (?, ?)";
            if (mysqli_stmt_prepare($stmt, $query3)) {
                ...
                foreach ($_POST["area"] as $idareas) {
                    if (!mysqli_stmt_execute($stmt)) {
                        header("Location:../be/pages/uploadCourseHei.php");
                        $_SESSION["course"] = 2;
                    }
                }
            }
