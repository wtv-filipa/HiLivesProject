...
$query = "UPDATE users 
SET description_es = ?, work_xp_es = ?, educ_lvl_ideduc_lvl = ?, active_person = ?, status_create = ?
WHERE idusers = ?";
...
if (isset($_GET["create"]) && !empty($_POST["esc"]) && !empty($_POST["area"]) && !empty($_POST["exp_t"]) && !empty($_POST["capacity"]) && !empty($_POST["environment"]) && !empty($_POST["def"])) {
    ...
    if (mysqli_stmt_prepare($stmt, $query)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
                ...
                if (mysqli_stmt_prepare($stmt2, $query2)) {
                    ...
                    foreach ($_POST["area"] as $idAreas) {
                        if (!mysqli_stmt_execute($stmt2)) {
                            header("Location: ../es/pages/createProfileTutor.php?create=$idCreate");
                            $_SESSION["create"] = 1;
                        }
                    }
                }
            ...
            //SUCESSO
            header("Location: ../es/pages/IndividualReqCreateTutor.php?create=$idCreate");
            $_SESSION["create"] = 1;
        } else {
            header("Location: ../es/pages/createProfileTutor.php?create=$idCreate");
            $_SESSION["create"] = 1;
