...
if (isset($_SESSION["idUser"]) && !empty($_POST["nomeuc"]) && !empty($_POST["uniuc"]) && !empty($_POST["data"])) {
    ...
    $query = "INSERT INTO done_cu (cu_name_es, university_name_es, date_cu, users_idusers) VALUES (?,?,?,?)";
    ...
    if (mysqli_stmt_prepare($stmt, $query)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            header("Location: ../es/pages/profile.php?user=$users_idusers");
            $_SESSION["profile"] = 3;
        } else {

            header("Location: ../es/pages/uploadCourse.php");
            $_SESSION["doneCU"] = 1;
        }
    } else {
