...
if ($_FILES['fileToUpload']['size'] != 0) {
    ...
    if (isset($_POST["but_upload"])) {
        ...
        if ($check !== false) {
            ...
            header("Location: ../pt/pages/uploadVacancy.php");
            $_SESSION["vac"] = 3;
            $uploadOk = 0;
        }
    }
            ...
            if (!empty($_GET["vac"]) && !empty($_POST["nomevaga"]) && !empty($_POST["descricao"]) && !empty($_POST["numvagas"]) && !empty($_POST["requisitos"]) && !empty($_POST["area"]) && !empty($_POST["jornada"]) && !empty($_POST["educ"]) && !empty($_POST["regiao"])) {
             ...
             VALUES (?,?,?)";
                ...
                if (mysqli_stmt_prepare($stmt, $query)) {
                    ...
                    $query = "INSERT INTO vacancies (vacancy_name, description_vac, free_vac, requirements, company_id, region_idregion, workday_idworkday, content_idcontent, educ_lvl_ideduc_lvl, areas_idareas) VALUES (?,?,?,?,?,?,?,?,?,?)";
                    ...
                    if (mysqli_stmt_prepare($stmt1, $query)) {
                        ...
                        if (mysqli_stmt_execute($stmt1)) {
                            ...
                            header("Location: ../pt/pages/allVacanciesComp.php");;
                            $_SESSION["vac"] = 1;
                        } else {
                            header("Location: ../pt/pages/uploadVacancy.php");
                              $_SESSION["vac"] = 2;
    ...
    if (!empty($_GET["vac"]) && !empty($_POST["nomevaga"]) && !empty($_POST["descricao"]) && !empty($_POST["numvagas"]) && !empty($_POST["requisitos"]) && !empty($_POST["area"]) && !empty($_POST["jornada"]) && !empty($_POST["educ"]) && !empty($_POST["regiao"])) {
        ...
        $link = new_db_connection();
        $stmt = mysqli_stmt_init($link);
        $query = "INSERT INTO vacancies (vacancy_name, description_vac, free_vac, requirements, company_id, region_idregion, workday_idworkday, educ_lvl_ideduc_lvl, areas_idareas) VALUES (?,?,?,?,?,?,?,?,?)";
