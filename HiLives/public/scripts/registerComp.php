...
if (!empty($_POST["nome"]) && !empty($_POST["email"]) && !empty($_POST["password"]) && !empty($_POST["data_nasc"]) && !empty($_POST["phone"]) && !empty($_POST["site"]) && !empty($_POST["desc"])) {
    ...
    $query = "INSERT INTO users (name_user, email_user, contact_user, birth_date, description, password, website, facebook, instagram, user_type_iduser_type) VALUES (?,?,?,?,?,?,?,?,?,?)";
            ...
            if (!empty($_POST["regiao"])) {
                ...
                if (mysqli_stmt_prepare($stmt2, $query2)) {
                    ...
                    if (!mysqli_stmt_execute($stmt2)) {
                        header("Location: ../pt/pages/register_comp.php");
                        $_SESSION["register"] = 1;
                    }
